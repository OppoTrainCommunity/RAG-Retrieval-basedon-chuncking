...
# -----------------------------
def build_recruitment_chat_messages(
    context: str,
    user_question: str,
    conversation_summary: str = "",
) -> List[BaseMessage]:
    """
    Recruitment helper chatbot.
    - MUST use retrieved context only
    - MUST cite file_name + chunk_id for claims
    - If not found, say so.
    """
    system_prompt = (
    ...
    # optional: lightweight memory so it feels like a chat, without leaking beyond context
    user_prompt = f"""
...
def openrouter_chat_completion(
    ...
    try:
        r = model_instance.invoke(messages)
...
with st.sidebar:
    ...
    max_retries = st.slider("max retries", 1, 6, 4, 2)
...
if user_text:
    ...
    try:
        ...
        # 3) prompt + llm
        lc_messages = build_recruitment_chat_messages(
            context=context,
